...
import com.drinks.erp.datasource.entities.DepotHead;
import com.drinks.erp.datasource.entities.DepotHeadVo4Body;
...
import com.drinks.erp.utils.*;
import com.drinks.erp.service.depotHead.DepotHeadService;
import com.drinks.erp.utils.*;
...
public class DepotHeadController {
     ...
     * @return
     */
    @GetMapping(value = "/findInOutDetail1")
    @ApiOperation(value = "临期商品明细接口")
    public BaseResponseInfo findInOutDetail1(@RequestParam("currentPage") Integer currentPage,
                                            @RequestParam("pageSize") Integer pageSize,
                                            @RequestParam(value = "organId", required = false) Integer oId,
                                            @RequestParam("number") String number,
                                            @RequestParam("materialParam") String materialParam,
        ...
        try {
            ...
            beginTime = Tools.parseDayToTime(beginTime, BusinessConstants.DAY_FIRST_TIME);
            endTime = Tools.parseDayToTime(endTime,BusinessConstants.DAY_LAST_TIME);
            List<DepotHeadVo4InDetail> list = depotHeadService.findInOutDetail1(beginTime, endTime, type, creatorArray, organArray,
                    StringUtil.toNull(materialParam), depotList, oId, StringUtil.toNull(number), remark, (currentPage-1)*pageSize, pageSize);
            int total = depotHeadService.findInOutDetailCount1(beginTime, endTime, type, creatorArray, organArray,
                    StringUtil.toNull(materialParam), depotList, oId, StringUtil.toNull(number), remark);
            map.put("total", total);
            //存放数据json数组
            if (null != list) {
